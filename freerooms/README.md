...
Then finally, with the provided image of a mobile app, **you will be tasked to recreate it**.

## Requirements

It is highly recommended to complete these tasks using Xcode on a Mac. Code can be written on Windows but running the tests (for question 1) will require a Mac - in particular, you will need to have at least version 16.0 of Xcode to run tests.
...
5. Errors should be handled accordingly.
   - There shouldn't be _any_ force-unwrap `!` or `fatalError` in your implementation (this excludes all the testing code).
   - There shouldn't be empty `catch` blocks.
   - There shouldn't be any `print` statements, such as `print(error)`.

6. The `Building` struct should _not_ implement `Decodable` - even in extensions.
...
The goal of this part is for you to demonstrate your Swift and SwiftUI skills, specifically utilising modern techniques and tools in Swift and SwiftUI to recreate the image below:

<img src="buildings-page.png" style="max-width: 30%; height: auto;">

You are given the freedom to choose how you recreate this.

- You may decide to use a single swift file or attach a newly created project in `mobile` folder.
- You can also use any external framework or library to aid your development.
- The following do not need to work:
  - The search bar
  - sorting by ascending/descending order and switching to card view (top-right icons)
  - switching between tabs
  - clicking on any building in the list
...
- You do not need to find exact matches for the fonts, icons or logos as **we expect you to utilise Apple's own library of icons, SF SYMBOLS**
- You do not need to aim for perfection. Please only complete as much of this exercise as you wish focusing on demonstrating the breadth of your mobile knowledge. There is no requirement for perfection or pixel-precise matching.

The sample data shown in the image is provided both in the `FreeroomsAssessment/Resources` folder.
...
This task is used to assess your understanding of **SwiftUI**, UI layout, and Apple’s design ecosystem. Pixel-perfect accuracy is **not required**.

### What we’re looking for

#### SwiftUI usage

- Appropriate use of SwiftUI views and layout containers (e.g. `VStack`, `HStack`, `List`, `LazyVStack`).
- Use of SwiftUI-style patterns rather than UIKit-style imperative code.

#### Layout & structure

- The screen should roughly match the provided design, including:
  - A header or navigation area
  - A search bar area
  - A list or grid of buildings
  - A tab bar or bottom navigation
- Focus on overall structure and layout rather than fine visual details.

#### Data usage

- Sample data from `BuildingData.json` should be used sensibly.
- Separating data models from views is encouraged, but not required.

#### Component reuse

- Break UI into smaller components where it improves clarity or reuse (e.g. a building row or card).
- Avoid unnecessary duplication of UI code.

#### Platform conventions

- Prefer Apple-provided SwiftUI components where possible.
- Use **SF Symbols** instead of custom icons.
- Avoid re-creating components that already exist in SwiftUI unless there is a clear reason.

#### Code quality

- Code should be readable, well-organised, and consistently styled.
- Naming should be clear and meaningful.
- The project or file should compile without warnings.

#### Scope

- This task is intentionally open-ended.
- Partial implementations are acceptable.
- The goal is to demonstrate understanding, not perfection.
